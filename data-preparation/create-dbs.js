...
promisified_1
    ...
    .then(function (warenkorbProducts) {
        return promisified_1
            .readdir('./presta/')
            .then(function (files) {
                return files.filter(function (x) {
                    return !!x.match(filenameRegex);
                });
            })
            .then(function (files) {
                return { warenkorbProducts: warenkorbProducts, files: files };
            });
    })
    .then(function (x) {
        return x.files.map(function (filename) {
            ...
            return request
                ...
                .then(function () {
                    return promisified_1.readFile('./presta/erheber__' + username + '.json');
                })
                .then(function (buffer) {
                    var data = JSON.parse(buffer.toString());
                    var erheber = _.assign({}, data.erheber, { _id: 'erheber' });
                    var preismeldestellen = data.preismeldestellen.map(function (x) {
                        return _.assign({}, x, { _id: pmsUri({ pmsNummer: x.pmsNummer }) });
                    });
                    var products = data.preismeldungen.map(function (x) {
                        return _.assign(x, {
