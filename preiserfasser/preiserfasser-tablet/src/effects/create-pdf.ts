...
export class CreatePdfEffects {
    ...
    constructor(private actions$: Actions, private pefLanguageService: PefLanguageService, private translateService: TranslateService, private store: Store<fromRoot.AppState>, private file: File, private platform: Platform) {
    }

    @Effect()
    pmsToPdf$ = this.actions$
        .ofType('CREATE_PMS_PDF')
        .flatMap(({ payload: { preismeldestelle, erhebungsmonat } }: { type: string, payload: { preismeldestelle: P.Models.Preismeldestelle, erhebungsmonat: string } }) =>
            Observable.of({ type: 'PDF_RESET_PMS' }).concat(
                this.store.select(fromRoot.getPreismeldungen).skip(1).filter(x => x.length > 0).take(1).map(preismeldungen => ({ preismeldestelle, erhebungsmonat, preismeldungen }))
                    .withLatestFrom(this.store.select(fromRoot.getPriceCountStatuses), this.store.select(fromRoot.getWarenkorb), this.pefLanguageService.currentLanguage$, (data, priceCountStatuses, warenkorb, currentLanguage) => ({ ...data, priceCountStatuses, warenkorb, currentLanguage }))
                    .flatMap(({ preismeldungen, erhebungsmonat, preismeldestelle, priceCountStatuses, warenkorb, currentLanguage }) => {
                        const translateFn = (key) => this.translateService.instant(key);
                        const data = mapData(preismeldungen, priceCountStatuses, warenkorb, currentLanguage, translateFn);
                        return toPdf(data, this.file, preismeldestelle, this.platform, erhebungsmonat, currentLanguage, translateFn)
                    })
    ...
    const test = preismeldungen.map(bag => [
        {
            ...
            col5: [translateFn('label_print_pm-id'), `${(priceCountStatuses[bag.warenkorbPosition.gliederungspositionsnummer] || {} as any).numActivePrices}/${(priceCountStatuses[bag.warenkorbPosition.gliederungspositionsnummer] || {} as any).anzahlPreiseProPMS}`],
        },
        {
            ...
            col5: [translateFn('label_print_stichtag'), `${bag.refPreismeldung.erhebungsAnfangsDatum ? bag.refPreismeldung.erhebungsAnfangsDatum + ' ' : ''}[${bag.preismeldung.erhebungsZeitpunkt || '–'}]`],
        },
        {
            ...
            col4: [translateFn('label_print_menge'), `${mengeFormatFn(bag.refPreismeldung.menge)} ${bag.warenkorbPosition.standardeinheit.de}`],
        ...
        {
            col1: [translateFn('label_print_bemerkungen'), bag.refPreismeldung.bemerkungen || '–']
        },
        ...(
            bag.warenkorbPosition.productMerkmale.length > 0 ?
                [
                    {
                        col1: [bag.warenkorbPosition.productMerkmale.map(x => x[currentLanguage]).join('; '), bag.preismeldung.productMerkmale.join('; ')]
                    }
                ]
                : []
        ),
        {
            col1: `${translateFn('label_print_preis-vor-reduktion')}: ${preisFormatFn(bag.refPreismeldung.basisPreis)} / ${translateFn('label_print_menge-vor-reduktion')}: ${mengeFormatFn(bag.refPreismeldung.menge)} ` +
            ` / ${translateFn('label_print_anzahl-code-r')}: ${bag.refPreismeldung.fehlendePreiseR.length} / ${translateFn('label_print_standardmenge')}: ${mengeFormatFn(bag.warenkorbPosition.standardmenge)}`,
        }
    ]);
}
    ...
    docA.autoTable([
        ...
        rawData,
        {
            ...
            createdCell: function (cell, opts) {
                ...
                if (opts.row.index == 0 && opts.column.index >= 1 && opts.column.index < 4 ||
...
function toPdf(data, file: File, preismeldestelle: P.Models.Preismeldestelle, platform: Platform, erhebungsmonat: string, currentLanguage: string, translateFn: (key: string) => string) {
    ...
    for (let i = 1; i < (pageCount + 1); i += 1) {
        ...
        doc.text(settings.page.margin.left, settings.page.margin.top, `${preismeldestelle.pmsNummer} ${preismeldestelle.name}`);
        ...
        doc.text(doc.internal.pageSize.width - 10, settings.page.margin.top, `${translateFn('label_print_seite')} ` + String(currentPage) + ' / ' + String(pageCount), null, null, 'right');
