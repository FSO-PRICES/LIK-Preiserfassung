...
export class PreismeldungInfoPopoverLeft extends ReactiveComponent implements OnChanges {
    ...
    constructor(elementRef: ElementRef, public sanitizer: DomSanitizer, @Inject(WINDOW) private wndw: Window) {
        ...
        this.popoverActive$ = this.buttonClicked$.pipe(
            map((x) => ({ type: 'TOGGLE' as const })),
            merge(
                this.observePropertyCurrentValue<any>('forceClose').pipe(
                    map((_) => ({ type: 'SET' as const, value: false })),
                ),
            ),
            scan<{ type: 'TOGGLE' } | { type: 'SET'; value: boolean }, boolean>((active: boolean, v) => {
        ...
        this.popoverWidth$ = recalcPopover$
            ...
            .pipe(
                map(
                    () =>
                        `calc(${elementRef.nativeElement.offsetLeft}px + ${elementRef.nativeElement.offsetWidth}px + ${
                            this.extraWidth || '0px'
                        } - ${this.pefRelativeSize(wndw.innerWidth, 1)})`,
                ),
            );
