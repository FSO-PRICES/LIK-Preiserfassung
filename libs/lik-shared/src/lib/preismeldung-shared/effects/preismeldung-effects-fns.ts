...
export function createVorReduktionProperties(bag: P.PreismeldungBag): {
    ...
    throw new Error(
        `Error in createVorReduktionProperties. Action: ${bag.preismeldung.aktion}, Bearbeitungscode: ${bag.preismeldung.bearbeitungscode}`,
    );
}
...
export function propertiesFromCurrentPreismeldung(bag: P.CurrentPreismeldungBag) {
    return {
        ...
        modifiedAt: format(new Date(), "yyyy-MM-dd'T'HH:mm:ss.SSSxxx"),
