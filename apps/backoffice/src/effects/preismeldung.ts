...
import { flatMap as arrayFlatMap, assign } from 'lodash';
...
export class PreismeldungEffects {
    ...
    loadPreismeldungenForPms$ = createEffect(() =>
        this.actions$.pipe(
            ofType('PREISMELDUNGEN_LOAD_FOR_PMS'),
            blockIfNotLoggedIn(this.store),
            flatMap(({ payload: pmsNummer }) =>
                loadPreismeldungenAndRefPreismeldungForPms({ pmsNummers: [pmsNummer] }),
            ),
    ...
    loadPreismeldungenForId$ = createEffect(() =>
        this.actions$.pipe(
    ...
    // TODO Fix types
    savePreismeldungPrice$ = createEffect(() =>
        this.actions$.pipe(
    ...
    savePreismeldungMessages$ = createEffect(() =>
        this.actions$.pipe(
    ...
    savePreismeldungAttributes$ = createEffect(() =>
        this.actions$.pipe(
    ...
    resetPreismeldung$ = createEffect(() =>
        this.actions$.pipe(
    ...
    deletePreismeldung$ = createEffect(() =>
        this.actions$.pipe(
            ofType('RESET_PREISMELDUNG'),
            blockIfNotLoggedInOrHasNoWritePermission<SimpleAction>(this.store),
            withLatestFrom(
                                ...
                                userDb
                                    ...
                                    .then((pmsPreismeldungenSort: P.Models.PmsPreismeldungenSort) => {
                                        const newPmsPreismeldungsSort = assign({}, pmsPreismeldungenSort, {
                                            sortOrder: pmsPreismeldungenSort.sortOrder.filter(
                                                (x) => x.pmId !== bag.pmId,
                                            ),
                                        });
                                        return userDb.put(newPmsPreismeldungsSort).then(() => newPmsPreismeldungsSort);
                                    })
                                    .then((pmsPreismeldungenSort) => ({
                                        pmId: bag.preismeldung._id,
                                        pmsPreismeldungenSort,
                                    })),
                            ),
                    ),
                ),
