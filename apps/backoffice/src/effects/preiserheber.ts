...
export class PreiserheberEffects {
    ...
    private errorCodes: { [code: string]: string } = {
        '409': this.translate.instant('label.pe.bereits_vorhanden'),
    };
            ...
            map((success) =>
                success
                    ...
                    : {
                          ...
                          payload: this.translate.instant('label.pe.password_ist_ungueltig'),
                      },
            ),
        ),
    );

    deletePreiserheber$ = createEffect(() =>
        this.actions$.pipe(
    ...
    savePreiserheber$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                flatMap((result) =>
                    loadPreiserheber(result.preiserheber._id).pipe(
                        map((preiserheber) => assign(result, { preiserheber })),
                    ),
                ),
                flatMap((result) =>
