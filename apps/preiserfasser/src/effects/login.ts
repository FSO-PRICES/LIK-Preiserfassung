            ...
            flatMap(() =>
                concat(
                    ...
                    this.getSettingsAndUsername().pipe(
                        switchMap(({ settings, username }) =>
                            E.match(settings.serverConnectionUrl, {
                                onLeft: () => of({ type: 'SET_IS_LOGGED_OUT' } as login.Action),
                                onRight: (url) =>
                                    getLoggedInUser(url, username).pipe(
                                        map((loggedInUser) =>
                                            !loggedInUser
                        ...
                        catchError(() => of({ type: 'SET_IS_LOGGED_OUT' } as login.Action)),
                    ),
                ),
            ),
        ),
