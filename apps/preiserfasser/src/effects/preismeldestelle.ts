...
export class PreismeldestelleEffects {
    ...
    loadPreismeldestellen$ = createEffect(() =>
        this.actions$.pipe(
            ...
            exhaustMap(() =>
                getDatabaseAsObservable().pipe(
                    ...
                    map((allDocs) => ({
                        type: 'PREISMELDESTELLEN_LOAD_SUCCESS',
                        payload: allDocs.rows.map((x) => x.doc),
                    })),
                            ...
                            db
                                .put(
                                    assign(
                                        {},
                                        doc,
                                        this.propertiesFromCurrentPreismeldestelle(currentPreismeldestelle),
                                    ),
                                )
                                .then(() => db),
                        )
