...
import { DropPreismeldungArg, ReactiveComponent, initDragula } from '@lik-shared';
...
export class PmsSortComponent extends ReactiveComponent implements OnChanges, OnDestroy, OnInit, AfterViewInit {
    ...
    @Output('preismeldung-sort-save')
    public preismeldungSortSave$: Observable<P.Models.PmsPreismeldungenSortProperties>;

    scrollList: P.PreismeldungBag[];
            ...
            this.onDrag$
                ...
                .subscribe((numPreismeldungenSelected) => {
                    ...
                    if (guMirror) {
                        ...
                        wndw.document.querySelector('.gu-mirror .message').innerHTML = `${
                            numPreismeldungenSelected || 1
                        } ${translateService.instant('label.pm.titel')}`;
                    }
                    this.el.nativeElement.querySelector('cdk-virtual-scroll-viewport').classList.add('is-dragging');
                }),
        );
    }

    preventScrollingWhileDragging(e: Event) {
        e.preventDefault();
    }

    public ngOnInit() {
        const scrollContainer = this.el.nativeElement.querySelector('cdk-virtual-scroll-viewport') as HTMLElement;
