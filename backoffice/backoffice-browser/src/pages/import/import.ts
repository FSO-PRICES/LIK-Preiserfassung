...
export class ImportPage implements OnDestroy {
    ...
    constructor(private store: Store<fromRoot.AppState>, private pefDialogService: PefDialogService) {
        ...
        this.warenkorbErhebungsmonat$ = this.store
            .select(fromRoot.getWarenkorbErhebungsmonat)
            .map(parseDate)
            .combineLatest(parsedWarenkorb$, (m, parsedWarenkorb) => (!!parsedWarenkorb ? null : m));
        this.preismeldestellenErhebungsmonat$ = this.store
            .select(fromRoot.getPreismeldestellenErhebungsmonat)
            .map(parseDate)
            .combineLatest(parsedPreismeldestellen$, (m, parsedWarenkorb) => (!!parsedWarenkorb ? null : m));
        this.preismeldungenErhebungsmonat$ = this.store
            .select(fromRoot.getPreismeldungenErhebungsmonat)
            .map(parseDate)
        ...
        this.warenkorbImportedCount$ = store
            ...
            .map(x => (!x ? null : x.products.length));
        ...
        this.preismeldestelleFileSelected$.takeUntil(this.onDestroy$).subscribe(file =>
            store.dispatch({
                type: 'PARSE_FILE',
        ...
        this.preismeldungFileSelected$.takeUntil(this.onDestroy$).subscribe(file =>
            store.dispatch({
                type: 'PARSE_FILE',
        ...
        this.canImport$ = parsedData$
            .map(
                x => x.parsedPreismeldestellen !== null && x.parsedPreismeldungen !== null && x.parsedWarenkorb !== null
            )
            .startWith(false)
            .publishReplay(1)
            .refCount();

        const importRequested$ = this.import$
        ...
        this.resetFileInputs$ = importRequested$.map(() => ({}));
    }
