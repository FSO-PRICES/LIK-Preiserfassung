...
export class PouchService {
    ...
    public putUserToDatabase(dbName, users: P.CouchSecurity) {
        return Observable.fromPromise(
            this.getSettings().then(settings => Observable.ajax({
    ...
    public getAllDocumentsForPrefixFromDb<T extends P.CouchProperties>(db: PouchDB.Database<{}>, prefix: string): Promise<T[]> {
        return db.allDocs(assign({}, { include_docs: true }, this.getAllDocumentsForPrefix(prefix))).then(x => x.rows.map(row => row.doc)) as Promise<T[]>;
    }

    public getAllDocumentsForPrefixFromDbName<T extends P.CouchProperties>(dbName: string, prefix: string): Observable<T[]> {
        return this.getDatabaseAsObservable(dbName)
            .flatMap(db => this.getAllDocumentsForPrefixFromDb<T>(db, prefix));
    }

    public getAllDocumentsForKeysFromDb<T extends P.CouchProperties>(db: PouchDB.Database<{}>, keys: string[]): Promise<T[]> {
    ...
    public dropDatabase(dbName) {
        return this.getCouchDb(dbName).then(db => db.destroy().then(() => true).catch(() => false));
    }

    public dropLocalDatabase(dbName) {
        return this.getLocalCouchDb(dbName).then(db => db.destroy().then(() => true).catch(() => false));
    }
    ...
    public syncDb(dbName: string) {
        return this.dropDatabase(dbName).then(() => {
            return this.getSettings().then(settings => {
    ...
    private _checkIfDatabaseExists(dbName) {
        return this.getDatabase(dbName).then(() => true).catch(() => false);
    }

    public getSettings() {
        return this.getLocalDatabase(this.dbNames.setting)
            .then(db => db.allDocs(assign({}, { include_docs: true }))
    ...
    public loginToDatabase(credentials: { username: string, password: string }): Promise<PouchDB.Database<{}>> {
        return this.getSettings().then(settings => {
            ...
            return this.couchLogin(couch)(credentials.username, credentials.password)
                .then(x => {
    ...
    public loadAllPreismeldestellen() {
        return this.getAllDocumentsForPrefixFromUserDbs<P.Preismeldestelle>('pms/')
            .flatMap(preismeldestellen => this.getDatabaseAsObservable(this.dbNames.preismeldestelle)
                .flatMap(db => this.getAllDocumentsForPrefixFromDb<P.Preismeldestelle>(db, 'pms/'))
                .map(unassignedPms => {
    ...
    public loadAllPreismeldungen(pmsNummer: string = '') {
        return this.getAllDocumentsForPrefixFromUserDbs<P.Preismeldung>(`pm/${pmsNummer}`)
            .flatMap((preismeldungen: any[]) => this.getDatabaseAsObservable(this.dbNames.preismeldung)
                .flatMap(db => this.getAllDocumentsForPrefixFromDb<P.PreismeldungReference>(db, `pm-ref/${pmsNummer}`).then(pmRefs => keyBy(pmRefs, pmRef => this.getPreismeldungId(pmRef))))
                .map(pmRefs => preismeldungen.map(pm => assign({}, pm, { pmRef: pmRefs[this.getPreismeldungId(pm)] }) as P.Preismeldung & { pmRef: P.PreismeldungReference }))
            );
    }

    public loadAllPreiserheber() {
        return this.getAllDocumentsForPrefixFromUserDbs<P.Erheber>('preiserheber')
            .flatMap(preiserheber => this.getDatabaseAsObservable(this.dbNames.preiserheber)
    ...
    public loadPreiserheber(id: string) {
        return this.listUserDatabases()
            .flatMap(userDbNames => {
                ...
                if (userDbName) {
                    return this.getDatabaseAsObservable(userDbName)
                        .flatMap(db => this.getDocumentByKeyFromDb<P.Erheber>(db, 'preiserheber').then(pe => assign(pe, { _id: pe.username })))
                }
                return this.getDatabaseAsObservable(this.dbNames.preiserheber)
    ...
    public updatePreiserheber(preiserheber: P.Erheber) {
        return this.listUserDatabases()
            .flatMap(userDbNames => {
                const userDbName = userDbNames.find(dbName => dbName === this.getUserDatabaseName(preiserheber.username));
                if (userDbName) {
                    return this.getDatabaseAsObservable(userDbName).map(db => ({ db, updatedPreiserheber: assign({}, preiserheber, { _id: 'preiserheber' }) }))
                }
                return this.getDatabaseAsObservable(this.dbNames.preiserheber).map(db => ({ db, updatedPreiserheber: preiserheber }))
            })
    ...
    public getAllDocumentsForPrefixFromUserDbsKeyed<T extends P.CouchProperties>(prefix: string): Observable<{ [username: string]: T[] }> {
        return this.listUserDatabases()
            .flatMap(dbnames => Observable.from(dbnames)
                .flatMap(dbname => this.getAllDocumentsForPrefixFromDbName<T>(dbname, prefix).map(docs => ({ [dbname.substr(5)]: docs })))
                .reduce((acc, docs) => ({ ...acc, ...docs }), {})
            );
    }
